{
    ...
    const renderTasks = () => {
        ...
        <li class="list__item ${task.done && hideDoneTask ? "button--hidden" : ""}">
            <button class="button button--green js-done">
                ${task.done ? "✔" : ""}
            </button>
            <span class="list__position ${task.done ? " list__item--done" : ""}">
                ${task.content}
            </span>
            <button class="button button--red js-remove">
                🗑
            </button>
        </li>`
        ).join("");
    ...
    const renderButtons = () => {
        ...
        if (tasks.length > 0) {
            button = `
            <button class="button button__teal js-toggleHideDoneTasks">
                ${hideDoneTask ? "Pokaż ukończone" : "Ukryj Ukończone"}
            </button>
            <button class="button button__teal js-markAllTasksDone"${tasks.every(({ done }) => done) ? "disabled" : ""}>
                Ukończ wszystkie
            </button>`
        };
